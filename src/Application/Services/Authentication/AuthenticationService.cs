...
) : IAuthenticationService
{
    ...
    {
        var user = await _userService.ValidateRefreshTokenAsync(request.UserID, request.RefreshToken);
        if(user == null)
        {
            return null;
        }
    ...
    public async Task<Result<User>?> RegisterAsync(UserDto request)
    {
        ...
        if((await _userService.ExistsByUsernameAsync(request.Username)).Value)
        {
