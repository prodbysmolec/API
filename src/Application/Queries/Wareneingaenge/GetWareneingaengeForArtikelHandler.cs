...
public class GetWareneingaengeForArtikelHandler(IWareneingangService service) : IRequestHandler<GetWareneingaengeForArtikelQuery, Result<ListContainerDto<GetWareneingaengeForArtikelResponse>>>
{
    private readonly IWareneingangService _service = service;
    public async Task<Result<ListContainerDto<GetWareneingaengeForArtikelResponse>>> Handle(GetWareneingaengeForArtikelQuery request, CancellationToken cancellationToken)
    {
        ...
        var wareneingaenge = await _service.GetWareneingaengeForArtikelAsync(request._id);
