...
public class LieferantenControllerTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public LieferantenControllerTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task GetLieferantById_ExistingId_ReturnsOkResult()
    {
        ...
        var createResponse = await client.PostAsJsonAsync("/lieferanten", createRequest);
        createResponse.EnsureSuccessStatusCode();
        var createdLieferant = await createResponse.Content.ReadFromJsonAsync<LieferantDto>();
        Assert.NotNull(createdLieferant);

        // Act
        var response = await client.GetAsync($"/lieferanten/{createdLieferant.Id}");
    ...
    public async Task GetLieferantById_NonExistingId_ReturnsNotFound()
    {
        ...
        // Act
        var response = await client.GetAsync($"/lieferanten/{nonExistingId}");
    ...
    public async Task CreateLieferant_ValidRequest_ReturnsCreatedResult()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/lieferanten", request);
    ...
    public async Task UpdateLieferant_ExistingIdWithValidData_ReturnsOkResult()
    {
        ...
        // Act
        var response = await client.PutAsJsonAsync($"/lieferanten/{createdLieferant.Id}", updateRequest);
    ...
    public async Task DeactivateLieferant_ExistingId_ReturnsOkResult()
    {
        ...
        // Act
        var response = await client.PatchAsync($"/lieferanten/{createdLieferant.Id}/deactivate", null);

        // Assert
        response.EnsureSuccessStatusCode();

        // Überprüfen, ob der Lieferant deaktiviert wurde
        var getLieferantResponse = await client.GetAsync($"/lieferanten/{createdLieferant.Id}");
    ...
    public async Task DeleteLieferant_ExistingId_ReturnsOkResult()
    {
        ...
        // Act
        var response = await client.DeleteAsync($"/lieferanten/{createdLieferant.Id}");
    ...
    public async Task SearchLieferanten_WithValidTerm_ReturnsMatchingResults()
    {
        ...
        // Act
        var response = await client.GetAsync($"/lieferanten/search?suchbegriff={uniqueName}");
    ...
    public async Task GetAllLieferanten_NurAktive_ReturnsOnlyActiveLieferanten()
    {
        ...
        await client.PostAsJsonAsync("/lieferanten", aktivRequest);
        ...
        var inaktivResponse = await client.PostAsJsonAsync("/lieferanten", inaktivRequest);
        inaktivResponse.EnsureSuccessStatusCode();
        var inaktivLieferant = await inaktivResponse.Content.ReadFromJsonAsync<LieferantDto>();

        // Deaktivieren
        await client.PatchAsync($"/lieferanten/{inaktivLieferant?.Id}/deactivate", null);

        // Act
        var response = await client.GetAsync("/lieferanten?nurAktive=true");
